...
export async function POST(request: Request) {
  ...
  const row = {
    user_id: user.id,
    game_date: date,
    subject_id: subjectId.slice(0, 80),
    classification: choice.slice(0, 40),
    note: payload.note ? payload.note.slice(0, 1000) : null,
  };

  const { error } = await pocketbase
    .from("rubin_comet_catchers_classifications")
    .upsert([row], { onConflict: "user_id,game_date,subject_id" });
...
export async function GET(request: Request) {
  ...
  const { data, error } = await pocketbase
    .from("rubin_comet_catchers_classifications")
    .select("subject_id,classification,note,created_at")
    .eq("user_id", user.id)
    .eq("game_date", date);
