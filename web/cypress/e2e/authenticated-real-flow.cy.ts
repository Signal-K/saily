describe("Authenticated real backend flow", () => {
  it("signs in with CI-created user, posts in discuss, and finds it via search", () => {
    cy.env(["E2E_TEST_EMAIL", "E2E_TEST_PASSWORD", "E2E_TEST_RUN_TAG"]).then((env) => {
      const email = typeof env.E2E_TEST_EMAIL === "string" ? env.E2E_TEST_EMAIL : undefined;
      const password = typeof env.E2E_TEST_PASSWORD === "string" ? env.E2E_TEST_PASSWORD : undefined;
      const runTag = typeof env.E2E_TEST_RUN_TAG === "string" ? env.E2E_TEST_RUN_TAG : "e2e-run";
